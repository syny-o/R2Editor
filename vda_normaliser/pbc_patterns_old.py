...
#######################################################################################
# DRIVEAWAY
key = re.compile(r'( MEASUREMENT| CHARACTERISTIC)\s+(PbcIn[._]?DriveAway[A-Za-z]*Ind[a-z]*[^\s/]*)')
...
for signal in pbc_in_signals:
    key = re.compile(r'( MEASUREMENT| CHARACTERISTIC)\s+(PbcIn[._]?' + signal + r'[^\s/]*)')
    value = 'PbcIn' + signal
    patterns.update({key : value})

# HOST SW VERSION
for number in range(0, 6):
    p_key = f'( MEASUREMENT| CHARACTERISTIC)\\s+(PbcIn[\\._]?HostSoftwareVersion.?[_\\[]?0{number}\\]?[^\\s/]*)'
    p_value = 'PbcInHostSoftwareVersion_0' + str(number)
    patterns.update({re.compile(p_key): p_value})

    p_key = f'( MEASUREMENT| CHARACTERISTIC)\\s+(PbcIn[\\._]?HostSoftwareVersion.?[_\\[]?{number}\\]?\\D[^\\s/^"]*)'
    p_value = 'PbcInHostSoftwareVersion_0' + str(number) + ' '
    patterns.update({re.compile(p_key): p_value})
...
for number in range(0, 20):
    if number < 10:
        p_key = f'( MEASUREMENT| CHARACTERISTIC)\\s+(PbcIn[\\._]?FaultRecoveryRequest.?[_\\[]?0{number}\\]?[^\\s/]*)'
        p_value = 'PbcInFaultRecoveryRequest_0' + str(number)
        patterns.update({re.compile(p_key): p_value})

        p_key = f'( MEASUREMENT| CHARACTERISTIC)\\s+(PbcIn[\\._]?FaultRecoveryRequest.?[_\\[]?{number}\\]?\\D[^\\s/^"]*)'
        p_value = 'PbcInFaultRecoveryRequest_0' + str(number) + ' '
        patterns.update({re.compile(p_key): p_value})

    else:
        p_key = f'( MEASUREMENT| CHARACTERISTIC)\\s+(PbcIn[._]?FaultRecoveryRequest.?_?\\[?{number}\\]?[^\\s/]*)'
        p_value = 'PbcInFaultRecoveryRequest_' + str(number)
        patterns.update({re.compile(p_key) : p_value})

# MOTOR CURRENT LEFT/RIGHT
for number in range(0, 10):
    p_key = f'( MEASUREMENT| CHARACTERISTIC)\\s+(PbcIn[\\._]?MotorCurrentLeft.?[_\\[]?0{number}\\]?[^\\s/]*)'
    p_value = 'PbcInMotorCurrentLeft_0' + str(number)
    patterns.update({re.compile(p_key): p_value})

    p_key = f'( MEASUREMENT| CHARACTERISTIC)\\s+(PbcIn[\\._]?MotorCurrentLeft.?[_\\[]?{number}\\]?\\D[^\\s/^"]*)'
    p_value = 'PbcInMotorCurrentLeft_0' + str(number) + ' '
    patterns.update({re.compile(p_key): p_value})

    p_key = f'( MEASUREMENT| CHARACTERISTIC)\\s+(PbcIn[\\._]?MotorCurrentRight.?[_\\[]?0{number}\\]?[^\\s/]*)'
    p_value = 'PbcInMotorCurrentRight_0' + str(number)
    patterns.update({re.compile(p_key): p_value})

    p_key = f'( MEASUREMENT| CHARACTERISTIC)\\s+(PbcIn[\\._]?MotorCurrentRight.?[_\\[]?{number}\\]?\\D[^\\s/^"]*)'
    p_value = 'PbcInMotorCurrentRight_0' + str(number) + ' '
    patterns.update({re.compile(p_key): p_value})

# MOTOR VOLTAGE LEFT/RIGHT
for number in range(0, 10):
    p_key = f'( MEASUREMENT| CHARACTERISTIC)\\s+(PbcIn[\\._]?MotorVoltageLeft.?[_\\[]?0{number}\\]?[^\\s/]*)'
    p_value = 'PbcInMotorVoltageLeft_0' + str(number)
    patterns.update({re.compile(p_key): p_value})

    p_key = f'( MEASUREMENT| CHARACTERISTIC)\\s+(PbcIn[\\._]?MotorVoltageLeft.?[_\\[]?{number}\\]?\\D[^\\s/^"]*)'
    p_value = 'PbcInMotorVoltageLeft_0' + str(number) + ' '
    patterns.update({re.compile(p_key): p_value})

    p_key = f'( MEASUREMENT| CHARACTERISTIC)\\s+(PbcIn[\\._]?MotorVoltageRight.?[_\\[]?0{number}\\]?[^\\s/]*)'
    p_value = 'PbcInMotorVoltageRight_0' + str(number)
    patterns.update({re.compile(p_key): p_value})

    p_key = f'( MEASUREMENT| CHARACTERISTIC)\\s+(PbcIn[\\._]?MotorVoltageRight.?[_\\[]?{number}\\]?\\D[^\\s/^"]*)'
    p_value = 'PbcInMotorVoltageRight_0' + str(number) + ' '
    patterns.update({re.compile(p_key): p_value})
...
for number in range(0, 99):
    if number < 10:
        p_key = f'( MEASUREMENT| CHARACTERISTIC)\\s+(PbcIn[\\._]?DataStorageRead.?[_\\[]?0{number}\\]?[^\\s/]*)'
        p_value = 'PbcInDataStorageRead_0' + str(number)
        patterns.update({re.compile(p_key): p_value})

        p_key = f'( MEASUREMENT| CHARACTERISTIC)\\s+(PbcIn[\\._]?DataStorageRead.?[_\\[]?{number}\\]?\\D[^\\s/^"]*)'
        p_value = 'PbcInDataStorageRead_0' + str(number) + ' '
        patterns.update({re.compile(p_key): p_value})

    else:
        p_key = f'( MEASUREMENT| CHARACTERISTIC)\\s+(PbcIn[._]?DataStorageRead.?_?\\[?{number}\\]?[^\\s/]*)'
...
# SW VERSION SIGNALS
for number in range(0, 6):
    p_key = f'( MEASUREMENT| CHARACTERISTIC)\\s+(PbcOut[\\._]?PbcSoftwareVersion.?[_\\[]?0{number}\\]?[^\\s/]*)'
    p_value = 'PbcOutPbcSoftwareVersion_0' + str(number)
    patterns.update({re.compile(p_key): p_value})

    p_key = f'( MEASUREMENT| CHARACTERISTIC)\\s+(PbcOut[\\._]?PbcSoftwareVersion.?[_\\[]?{number}\\]?\\D[^\\s/^"]*)'
    p_value = 'PbcOutPbcSoftwareVersion_0' + str(number) + ' '
    patterns.update({re.compile(p_key): p_value})
...
for signal_key, signal_value in pbc_out_signals.items():
    key = re.compile(r'( MEASUREMENT| CHARACTERISTIC)\s+(PbcOut[._]?' + signal_key + r'[^\s/]*)')
    value = signal_value
    patterns.update({key : value})
...
for number in range(0, 20):
    if number < 10:
        p_key = f'( MEASUREMENT| CHARACTERISTIC)\\s+(PbcOut[\\._]?FaultStatus.?[_\\[]?0{number}\\]?[^\\s/]*)'
        p_value = 'PbcOutFaultStatus_0' + str(number)
        patterns.update({re.compile(p_key): p_value})

        p_key = f'( MEASUREMENT| CHARACTERISTIC)\\s+(PbcOut[\\._]?FaultStatus.?[_\\[]?{number}\\]?\\D[^\\s/^"]*)'
        p_value = 'PbcOutFaultStatus_0' + str(number) + ' '
        patterns.update({re.compile(p_key): p_value})

    else:
        p_key = f'( MEASUREMENT| CHARACTERISTIC)\\s+(PbcOut[._]?FaultStatus.?_?\\[?{number}\\]?[^\\s/]*)'
...
for number in range(0, 99):
    if number < 10:
        p_key = f'( MEASUREMENT| CHARACTERISTIC)\\s+(PbcOut[\\._]?DataStorageWrite.?[_\\[]?0{number}\\]?[^\\s/]*)'
        p_value = 'PbcOutDataStorageWrite_0' + str(number)
        patterns.update({re.compile(p_key): p_value})

        p_key = f'( MEASUREMENT| CHARACTERISTIC)\\s+(PbcOut[\\._]?DataStorageWrite.?[_\\[]?{number}\\]?\\D[^\\s/^"]*)'
        p_value = 'PbcOutDataStorageWrite_0' + str(number) + ' '
        patterns.update({re.compile(p_key): p_value})

    else:
        p_key = f'( MEASUREMENT| CHARACTERISTIC)\\s+(PbcOut[._]?DataStorageWrite.?_?\\[?{number}\\]?[^\\s/]*)'
